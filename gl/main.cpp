 ...
 *
 * Usage: ./driver -i <input-file> [ -s <solution-file> ] [ -o <output-file> ] [ -f <stats-file> ]
 ...
 *
 * The -f flag is used for the grading script, so that grading will not be
...
{
    std::cerr<<"Usage: "<<prog_name<<" -i <input-file> [ -s <solution-file> ] [ -f <stats-file> ]"<<std::endl;
...
int main(int argc, char** argv)
{
    ...
    while(1)
    {
        int opt = getopt(argc, argv, "s:i:f:o:");
        if(opt==-1) break;
        switch(opt)
        {
    ...
    dump_png(state.image_color,state.image_width,state.image_height,output_file);
